...
SESSION_NAME = os.environ.get("OPENF1_SESSION_NAME", "").strip()
...
def resolve_session_keys() -> list[int]:
    """Resolve one or more session_keys based on config."""
    ...
    logger.info(f"Resolving sessions: year={YEAR}, meeting='{MEETING}', session_name='{SESSION_NAME}'")
    sessions = fetch_with_retry(f"{BASE_URL}/sessions", params)

    if not sessions:
        raise ValueError(f"No sessions found for params={params}")
    ...
    if not sessions:
        raise ValueError(f"No sessions matched meeting='{MEETING}', session_name='{SESSION_NAME}'")

    keys = [s["session_key"] for s in sessions]
    for s in sessions:
        logger.info(f"  session_key={s['session_key']} — {s.get('meeting_name')} / {s.get('session_name')} ({s.get('date_start', '')})")
    logger.info(f"Resolved {len(keys)} session(s)")
    return keys
...
def main():
    ...
    session_keys = resolve_session_keys()
    counts = {}
    ...
    for session_key in session_keys:
        logger.info(f"--- Importing session_key={session_key} ---")
        ...
        if INCLUDE_CAR_DATA:
            ...
            counts["car_data"] = counts.get("car_data", 0) + car_count
            logger.info(f"  → {car_count} car_data records for session_key={session_key}")
        else:
